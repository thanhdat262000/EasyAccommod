...
module.exports.index = async(req, res) => {
    ...
    if(isEmpty(req.query)){
        ...
        connection.query(sql, [city_name, district_name, apartment_type] , (err, results, fields) => {
            try{
                if(err) throw err;
                // console.log(req.url);
                // req.url = `/`
                ...
                const data = results.filter(apartment => {
                    if(rent_min.length === 0){
                        if(rent_max.length === 0){
                            ...
                            return apartment.square <= square_max && apartment.square >= square_min;
                        }
                        return apartment.square <= square_max && apartment.square >= square_min && apartment.price <= rent_max;
                    }
                    return apartment.square <= square_max && apartment.square >= square_min && apartment.price <= rent_max && apartment.price >= rent_min;
                })
                res.json(data);
            }catch(err){
                console.log(err);
